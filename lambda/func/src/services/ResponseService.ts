...
class Response {
	...
	public clone(): Response {
		...
		return new Response({ code, text }, this._body, { ...this._headers });
	}
		...
		super(
			{
				...
				text: HttpStatusText.BAD_REQUEST,
			},
			body,
			headers
		);
		...
		super(
			{
				...
				text: HttpStatusText.INTERNAL_SERVER_ERROR,
			},
			body,
			headers
		);
...
export class ResponseService {
	...
	public ok(body: ResponseBody): ResponseObject {
		return this._toResponseObject(Response.withCORS(new OKResponse(body)));
	}

	public badRequest(msg: string, errors: string[] = []): ResponseObject {
		return this._toResponseObject(
			Response.withCORS(new BadRequestResponse({ msg, errors }))
		);
	}
	...
	public internalServerError(
		...
		return this._toResponseObject(
			Response.withCORS(new InternalServerErrorResponse({ msg, errors }))
		);
	}

	private _toResponseObject(response: Response): ResponseObject {
		...
		if (response.statusCode() !== 200) throw new Error(responseStr);
